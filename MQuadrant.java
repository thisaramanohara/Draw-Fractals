...
class MQuadrant extends JPanel implements Runnable {
	
	static Graphics g;	//g is common to all objects of MQuadrant
	int qNum;	//quadrant number
	double xGap,xMin,yGap,yMax,iter;	//definitions of these variables are same as before
	...
	public MQuadrant(int qNum,double xGap,double xMin,double yGap,double yMax,double iter) {
		//getting values
		this.qNum = qNum;
		this.xGap = xGap;
		this.xMin = xMin;
	...
	public void run() {
	    ...
	    for(int i=iStart;i<iEnd;i++) {
	    	...
	    	for(int j=jStart;j<jEnd;j++) {	
	    		...
	    		//if it is in Mandelbrot set
	    		if(iteration==this.iter) {
	    			//setting color black and call printpoint method in Mandelbrot class to print the point 
	    			Mandelbrot.printPoint((Graphics2D)g, Color.BLACK, i,j);
	    		}
	    		//otherwise setting different color to the point
	    		else {
	    			Color color = Color.getHSBColor((float)iteration/(float)iter, 1.0f, 1.0f);
	    			Mandelbrot.printPoint((Graphics2D)g, color, i, j);
	    		}
